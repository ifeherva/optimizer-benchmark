...
class RAdam_LRD(Optimizer):

    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, dropout=0.0):
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad, dropout=dropout)

        self.name = '{}_{}_{}'.format(lr, betas[0], betas[1])
        super(RAdam_LRD, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if N_sma >= 5:
                    ...
                    p_data_fp32.addcmul_(-1, exp_avg, lr_dropout)
                else:
